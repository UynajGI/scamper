...
| 2026-08-19 | Variational family L3 (nodeless DMC) | ✅ `DmcKernel`: drift-diffusion with exact Green-ratio Metropolis, `⌊g+u⌋` branching, E_T feedback, forward-walking pure estimators (lineage rings; credited once per surviving line = descendant weighting), `qmc-rs-dmc-v1` checkpoints with lineage serialization. **Two physics bugs caught by the gates and fixed**: (1) the backward Green displacement is `R−R'−τb(R')`, whose square has a + sign on the backward drift — flipping it parked the population on a wrong stationary manifold (DMC above VMC, diagnosed by an isolated move-stationary probe against the analytic ⟨r²⟩ of ψ² and a 1-D sandbox replication); (2) the exact-state Jastrow remainder is `a = (Ω−ω)/4`, not `Ω/4` (the GaussianTrap already carries part of the relative gaussian). Gates: DMC converges to the derived separable `E₀ = (3/2)(ω+√(ω²+2k))` from an approximate nodeless ψ_T, strictly closer than VMC; exact-ψ_T identity (mixed = pure = E₀ at 1e-10); population-control bias shrinks 8→64 walkers at equal budgets; bit-identical same-seed runs, checkpoint round-trip/replay, corruption/mismatch/validation rejections. Validated domain: nodeless states; fixed-node enforcement (signed amplitudes + crossing rejection) is the named follow-up. 269 tests (262 + 7 long) |

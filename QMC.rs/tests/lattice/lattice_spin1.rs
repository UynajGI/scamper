...
fn s1_heisenberg_chain_produces_finite_results() {
    ...
    match model {
        Ok(model) => {
            ...
            // S=1 AFM Heisenberg: energy should be negative
            assert!(
                energy.is_finite(),
        ...
        Err(e) => {
            // If the model builder rejects S=1, that's also informative
            eprintln!("S=1 model construction failed: {e}");
            eprintln!("This may indicate S>1/2 is not supported for this coupling");
        }
    }
}

...
///
/// Every sweep draws a fresh occupancy configuration: sites
/// (`mode = "site"`, default) or bonds (`mode = "bond"`) are open
/// independently with probability `p`. Samples are i.i.d., so there is no
/// Markov-chain equilibration — set `thermalization_sweeps = 0` and read
/// every sweep as one independent sample.
///
/// Measured observables: `Occupied` (open sites or open bonds), `MaxCluster`
/// (largest cluster in sites), `SecondMoment` (`sum(s_i^2)`), `NClusters`
/// and `Spanning` (0/1 indicator whose mean is the crossing probability).
///
...
impl PercolationMC {
    /// Build a percolation ensemble on `lattice`.
    pub fn new(
        lattice: CsrLattice,
        ...
        if !p.is_finite() || !(0.0..=1.0).contains(&p) {
            return Err(format!(
                "occupation probability must be finite within [0, 1], got {p}"
            ));
        }
...
impl FromParams for PercolationMC {
    fn validate_params(params: &Params) -> Result<(), CarloError> {
        let p = parse_param::<f64>(params, "p")?
            .ok_or_else(|| invalid("p", "missing required occupation probability"))?;
        if !p.is_finite() || !(0.0..=1.0).contains(&p) {
            return Err(invalid(
                "p",
                format!("must be finite within [0, 1], got {p}"),
            ));
        }
        if let Some(mode) = parse_param::<String>(params, "mode")? {
            PercolationMode::from_label(&mode).map_err(|error| invalid("mode", error))?;
        }
    ...
    fn from_params(params: &Params, _rng: &mut Self::Rng) -> Result<Self, CarloError> {
        ...
        let p = parse_param::<f64>(params, "p")?.expect("validated above");
        let mode = match parse_param::<String>(params, "mode")? {
            Some(label) => {
                PercolationMode::from_label(&label).map_err(|error| invalid("mode", error))?
            }
            None => PercolationMode::Site,
        };
        ...
        Self::new(lattice, mode, p, from, to).map_err(|error| invalid("percolation", error))
    }
}

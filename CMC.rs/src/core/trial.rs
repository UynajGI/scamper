...
#[inline]
pub fn metropolis_hastings_step<State, Model, Movement, Target, Rule, RngType>(
    state: &mut State,
    model: &Model,
...
where
    ...
    Rule: AcceptanceRule<<State as TrialEvaluator<Model, Movement>>::Delta>,
    RngType: Rng,
{
    let delta = state.evaluate_trial(model, &proposal.movement, patch);
    let log_acceptance =
        acceptance.log_acceptance(target, &delta, proposal.log_reverse_over_forward);
    assert!(

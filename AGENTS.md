...
| CMC.rs | Classical MC | Lattice + particle + generalized ensembles + classical worm (Ising HT graph) + classical dynamics (Kawasaki, Gillespie/BKL, hard-sphere event-chain) + site/bond percolation. `ClassicalMC` wrapper, `IsingGraphWormMC`, `KineticIsingMC`, `HardSphereEventChainMC`, `PercolationMC` Carlo.rs adapters |
...
| `percolation/` | `mod.rs`, `state.rs`, `cluster.rs`, `mc.rs` | `PercolationMode` (site/bond), `OccupancyState` (i.i.d. occupancy resampling), `UnionFind` + `cluster_stats` (max cluster, `sum(s_i^2)`, cluster count, spanning-set crossing), `PercolationMC` Carlo.rs adapter |

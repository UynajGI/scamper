...
Hooks via **lefthook** (`lefthook.yml` + `.lefthook/` scripts). pre-commit runs `fmt --check` → `cargo check` → `clippy` → `typos` on staged `.rs` (affected crates only); commit-msg enforces Conventional Commits; post-commit runs `codegraph sync` (non-blocking); pre-push runs `cargo deny` only (test moved to CI for fast pushes — run `just test` manually before pushing). Install: `lefthook install` or `just hooks`. Skip: `LEFTHOOK=0 git commit`. Lint policy (incl. `unsafe_code = "deny"`) is codified in `[workspace.lints]` (`Cargo.toml`) — no `RUSTFLAGS` needed; the sole exception is `Carlo.rs/src/backend/mpi.rs` (`#![allow(unsafe_code)]` for MPI FFI). CI (`.github/workflows/ci.yml`) runs fmt + clippy + test + deny (`--all-features`) as parallel jobs; `nightly.yml` runs the `--ignored` long tests daily plus a `zscore-monitor` job (`SCUTTLE_ZSCORE_SEEDS=64`; `just nightly-zscore` reproduces locally — z-score tests in all crates scale their seed counts through this env var, unset = defaults). `nightly.yml` also runs a `carlo-framework` job: the Carlo.rs full suite (`--all-features`) plus every MPI test under `mpirun` at np 1/2/4, each with its own `--exact` invocation (`just mpi-test [np]` reproduces locally — MPI cannot be initialized twice per process, so MPI tests must never share one cargo-test run). Release flow: development lands on `dev`, is fast-forwarded to `main` (the default branch), and releases are always tagged on `main` — pushing a `v*` tag triggers `.github/workflows/publish.yml` (validate → crates.io → GitHub release). Never cut tags from `dev`.
...
| CMC.rs | Classical MC | Lattice + particle + generalized ensembles + classical worm (Ising HT graph) + classical dynamics (Kawasaki, Gillespie/BKL, hard-sphere event-chain) + site/bond/mixed percolation. `ClassicalMC` wrapper, `IsingGraphWormMC`, `KineticIsingMC`, `HardSphereEventChainMC`, `PercolationMC` Carlo.rs adapters |
...
| `worm/` | `error.rs`, `model.rs`, `state.rs`, `kernel.rs`, `ising.rs`, `ensemble.rs`, `mc.rs` | Generic `WormKernel<M>` persisting open/close/step/bounce transitions, `WormState` physical/worm sector container, `IsingGraphWormModel` (HT graph, tanh weights), `IsingGraphWormEnsemble` (multi-component lattices: per-component two-defect worms on domain-separated streams), versioned JSON checkpoints `cmc-rs-ising-worm-v1`/`-v2`, `EndpointPairHistogram`, exact 2ⁿ enumeration (≤24 edges) |
...
| `percolation/` | `mod.rs`, `state.rs`, `cluster.rs`, `mc.rs` | `PercolationMode` (site/bond/mixed), `OccupancyState` (i.i.d. occupancy resampling), `UnionFind` + `cluster_stats` (max cluster, `sum(s_i^2)`, cluster count, spanning-set crossing), `PercolationMC` Carlo.rs adapter |
...
- Classical worm: `WormKernel<M>` drives open/close/step/bounce transitions in an extended physical/worm sector space. `WormModel` trait owns defects, proposals, `log_reverse_over_forward` Hastings factors, weight deltas and transactional patches. Open: `ln η + ln P_close + ln N`; close: exact negative; step: `log_weight_ratio + log_reverse_over_forward + ln(1-P_close·1_new_coincident) - ln(1-P_close·1_old_coincident)`. `EndpointPairHistogram` estimates two-point correlations via `count(tail,head)/count(tail,tail)`. `IsingGraphWormMC` implements `MonteCarlo`+`FromParams` with versioned `cmc-rs-ising-worm-v1`/`-v2` JSON checkpoints; `from_lattice` decomposes disconnected lattices into per-component worms (the raw model/kernel pair stays connected-only). Zero-field, non-negative `J`, self-loop rejection.
...
- Percolation: i.i.d. samples, not a Markov chain — every `sweep()` redraws occupancy (`mode`: sites with `p_site`, bonds with `p_bond`, or mixed where a bond connects only when open AND both endpoints open), `measure()` analyses the occupied subgraph via union-find; set `thermalization_sweeps = 0`. Crossing is tested between `spanning_from`/`spanning_to` site sets with structural defaults (square: left/right columns; chain: end sites) and explicit comma-separated lists for arbitrary graphs.

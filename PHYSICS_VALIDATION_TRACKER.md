...
| 2026-08-19 | CMC item-20: all 19 solvers production-ready | Multi-component worm **implemented** (`src/worm/ensemble.rs`, per-component two-defect worms on `RngStreamKey` domain-separated streams; validated vs full 2^8 enumeration + partition identity + cross-solver on two disjoint 4×4 squares; 4 suite + 3 lib tests); microcanonical over-relaxation production-validated **in composition** (machine-precision reflection identities + deterministic DB, exact XY-ring quadrature from 2 inits, Wolff cross-solver 8×8 O(2)/O(3), analytic limits; 6 tests); **every offered hybrid composition** validated (all six Ising pairings vs enumeration E/m²/C with pooled Σz, boundary semantics bit-exact, continuous O(2) pairings vs Wolff; 3 tests); MultiSpinIsing cross-solver vs scalar Metropolis + multi-seed z coverage + 64-replica consistency (2 tests); SW continuous-spin cluster updates validated vs quadrature/limits/Wolff (4 tests). CMC.rs now 292 suite tests (277 + 15 long) + 73 lib — see `MATURITY_ASSESSMENT.md` item 20 and `CMC.rs/VALIDATION.md` |
| 2026-08-19 | QMC production hardening: all 4 solvers production-ready | Generic-S scattering identities at 1e-12 for S ∈ {1/2,1,3/2,2,5/2} across the model catalog, both policies (`lattice_scattering_generic_s.rs`); occupation per-update DB — machine-precision heat-bath-kernel identity + empirical flow balance/stationary marginal — and BFS-proven update-graph connectivity (`occupation_detailed_balance.rs`, `occupation_ergodicity.rs` extension, lib test in `worldline.rs`); cluster ergodicity vs ED + sector visits (`cluster_ergodicity_ed.rs`); **multi-mode cluster baths validated** — mass-weighted kernel identity + 3-observable MC-vs-ED (`cluster_multimode.rs`, closes the old "single-mode only" limitation by validation, not documentation); criterion-G input-validation audit across all four solvers (8 rejection tests) with a **silent free-spin fallback fixed in source** (unknown model names previously compiled to zero-coupling XYZ; empty edge lists also now rejected — `lattice/mc.rs`). QMC.rs now 202 tests (195 + 7 long) — see `MATURITY_ASSESSMENT.md` and `QMC.rs/VALIDATION.md` |

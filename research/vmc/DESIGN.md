...
| **L2** | Optimizers: variance minimization, stochastic reconfiguration / natural gradient, linear method | SR reproduces analytic optimum on quadratic toy; linear-method step ≥ SR step; monotone convergence under trust region |
| **L3** | DMC (fixed-node): drift-diffuse + branching + population control; descendant-weighted pure estimators | Bosonic nodeless HO: DMC energy exact; DMC(fn) ≥ VMC bound statistically; population-control bias ∝ 1/N_w; pure vs mixed estimator agree at τ=0 |
